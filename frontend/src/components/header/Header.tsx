...
const Header = () => {
  const navigation = [
    { title: "Customers", path: "about" },
    { title: "Careers", path: "about" },
    { title: "Guides", path: "about" },
    { title: "Partners", path: "about" },
  ];
      ...
      <header className="flex shadow-lg py-4 px-4 sm:px-10 bg-white font-[sans-serif] min-h-[70px] tracking-wide relative z-50">
        <div className="flex flex-wrap items-center justify-between gap-4 w-full">
          <a
            ...
            href="#"
          >
            <h2 className="text-2xl font-semibold uppercase">For your Beauty</h2>
          </a>
          <div
            ...
            <ul className="lg:flex lg:gap-x-5 max-lg:space-y-3 max-lg:fixed max-lg:bg-white max-lg:w-1/2 max-lg:min-w-[300px] max-lg:top-0 max-lg:left-0 max-lg:p-6 max-lg:h-full max-lg:shadow-md max-lg:overflow-auto z-50">
              ...
              {navigation.map((data, index) => (
                <li key={index} className="max-lg:border-b max-lg:py-3 px-3">
                  <Link
                    to={`${data.path}`}
                    className="hover:text-[#007bff] text-[#333] block font-medium text-md"
                  >
                    {data.title}
                  </Link>
                </li>
